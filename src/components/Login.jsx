...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Store actual user data on successful login
        localStorage.setItem("user", JSON.stringify({
          email: data.email,
          username: data.username,
          profileImage: data.profileImage || '', // Include profileImage if your login response sends it
          location: data.location || '', // Include location and bio if your login response sends it
          bio: data.bio || ''
        }));
        navigate('/h');
      } else {
  ...
  const handleSkip = () => {
    // Set default guest user data in localStorage
    localStorage.setItem("user", JSON.stringify({
      email: " ", // A placeholder email
      username: " ",     // A placeholder username
      profileImage: "/profile.jpg", // A default image for guests
      location: "World",
      bio: "Exploring as a guest."
    }));
    alert('Browse as a guest!');
    navigate('/h'); // Navigate to the home/profile page
  };
  // --- END MODIFIED handleSkip FUNCTION ---
